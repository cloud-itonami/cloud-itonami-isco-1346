(ns branch-manager.advisor
  "BranchManagerAdvisor — proposes a branch management operation
  (schedule staffing, log a performance report, draft customer
  correspondence, flag a compliance concern) for a registered branch.
  ...
  store and has no notion of branch/customer provenance or regulatory risk;
  `branch-manager.governor` is the independent system that decides whether
  the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :schedule-staffing|:log-branch-performance-report|
         :draft-customer-correspondence|:flag-compliance-concern
...
(defn- infer
  ...
  {:op op
   ...
   :rationale (str "proposed " (name op) " for branch " (:branch-id request))})
...
(def ^:private system-prompt
  "You are a branch operations advisor for a financial/insurance
   institution. Given a branch management operation request, propose an
   :op, an honest :confidence (0.0-1.0), and a :stake (:low/:medium/:high).
   Never fabricate confidence you don't have. Never propose to approve
   loans, set rates, or bind the institution financially — only humans
   make those decisions. Propose staffing, reporting, correspondence, or
   compliance escalation only.")
