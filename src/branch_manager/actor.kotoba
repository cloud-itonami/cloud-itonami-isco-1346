(ns branch-manager.actor
  "BranchManagerActor — the ISCO-08 1346 financial/insurance branch
  manager actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one branch management request
  (intake → advise → govern → decide → commit/hold, with a human-approval
  interrupt for escalated proposals). No infinite internal loop;
  ...
  The unconditional invariant: the BranchManagerAdvisor can never
  directly commit a record or dispatch a robot action the
  BranchManagerGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [branch-manager.advisor :as advisor]
            [branch-manager.governor :as governor]
            [branch-manager.store :as store]))

(defn build-graph
  "Build a compiled BranchManagerActor graph. `store` implements
  `branch-manager.store/Store`. `advisor` implements
  `branch-manager.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:branch-id (:branch-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
...
(defn run-request!
  "Run one branch management request to completion or interrupt.
